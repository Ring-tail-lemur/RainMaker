...
SELECT
    target_pr.pr_id           AS pull_request_id,
...
FROM
    (
        SELECT pr.pull_request_id  AS pr_id,
        ...
        FROM pull_request_event pr_event
             ...
             (
                 SELECT
                     prd.outgoing_pull_request_id AS pr_id,
        ...
        WHERE pr_event.event_type = 'CLOSED'
          ...
          AND pr_source.source_count IS NULL
    ) target_pr
        JOIN
    (
    ...
    (
        ...
        where pr_commit_table.first_commit = 1
    ) commits
